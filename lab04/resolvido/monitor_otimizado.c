...
void* monitor_sensores(void* arg) {
    ...
    for (int i = 0; i < LEITURAS_POR_SENSOR; i++) {
        ...
        if (resultado_processado > limite) {
            ...
            // SEÇÃO CRÍTICA PROTEGIDA 1
            pthread_mutex_lock(args->alerta_mutex);
            (*args->alertas_globais)++;
            pthread_mutex_unlock(args->alerta_mutex);
            // Solução foi colocar o mutex imediatamente antes e
            // depois da linha que altera a variavel compartilhada
            ...
            // Solução foi colocar o mutex antes e depois do if. 
            // A seção crítica não poderia ser só a linha que altera a variável (log_offset) porque
            // ela também é utilizada na comparação do if.
            // Se os mutex fossem colocados exatamente antes e depois da alteração, poderia ocorrer
            // de outra thread entrar no if quando na verdade não deveria (outra altera o valor depois
            // dessa entrar)
        }
